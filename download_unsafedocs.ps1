...
<#
.SYNOPSIS
    UNSAFE-DOCS Downloader - PowerShell version for Windows
    Downloads ~5.3M PDFs + 180K other files from Common Crawl via Digital Corpora

.DESCRIPTION
    Standalone script for downloading the UNSAFE-DOCS corpus (CC-MAIN-2021-31-UNSAFE).
...
.PARAMETER Path
    Custom download path (default: S:\UNSAFE-DOCS)
...
.EXAMPLE
    .\download_unsafedocs.ps1 -List
    
.EXAMPLE
    .\download_unsafedocs.ps1 -Tier sample
    
.EXAMPLE
    .\download_unsafedocs.ps1 -Tier medium -Parallel 8
    
.EXAMPLE
    .\download_unsafedocs.ps1 -Tier complete -Path "D:\Data\UNSAFE-DOCS"
    
.EXAMPLE
    .\download_unsafedocs.ps1 -Limit 50000 -Parallel 8
#>
...
$S3Prefix = 'corpora/files/CC-MAIN-2021-31-UNSAFE/'
$MaxRetries = 3
...
if ($List) {
    Write-Host "`nUNSAFE-DOCS Download Tiers:" -ForegroundColor Cyan
    Write-Host "=" * 70
    ...
    Write-Host "=" * 70
    Write-Host "`nSource: Common Crawl (CC-MAIN-2021-31-UNSAFE)"
    Write-Host "Total: ~5.3M PDFs + 180K other files from Common Crawl"
    exit 0
}
...
if ($Tier) {
    ...
    $FileLimit = $Limit
    $SizeEstimate = "~$([math]::Round($Limit / 10000, 1.2)) GB"
    $Description = "Custom limit"
}

# Add UNSAFE-DOCS subfolder unless already in path
if ($Path -notlike "*UNSAFE*") {
    $BaseDir = Join-Path $Path "UNSAFE-DOCS"
}
else {
    $BaseDir = $Path
}
...
Write-Host "UNSAFE-DOCS DOWNLOADER" -ForegroundColor Cyan
Write-Host ("=" * 60) -ForegroundColor Cyan
Write-Host "Source:           Common Crawl (CC-MAIN-2021-31-UNSAFE)"
Write-Host "Total Available:  ~5.3M PDFs + 180K other files"
...
for ($i = 0; $i -lt $total; $i += $batchSize) {
    ...
    $batch | ForEach-Object -Parallel {
        ...
        Write-Progress -Activity "Downloading UNSAFE-DOCS" -Status "$completed / $total files ($pct%)" `
            -PercentComplete $pct -CurrentOperation "DL: $($stats.Downloaded) | Skip: $($stats.Skipped) | Fail: $($stats.Failed)"
    }
}

Write-Progress -Activity "Downloading UNSAFE-DOCS" -Completed
